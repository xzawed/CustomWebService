...
describe('ProjectService.create()', () => {
  ...
  beforeEach(async () => {
    ...
    projectRepo.create.mockResolvedValue({ id: 'proj-1', userId: 'user-1', name: '테스트', context: validContext })
    projectRepo.insertProjectApis.mockResolvedValue(undefined)
  })

  it('API 선택이 0개면 ValidationError를 던진다', async () => {
    await expect(
      service.create('user-1', { ...validInput, apiIds: [] })
  ...
  it('프로젝트가 20개 이상이면 ValidationError를 던진다', async () => {
    ...
    await expect(
      service.create('user-1', validInput)
    ).rejects.toThrow(ValidationError)
  })
