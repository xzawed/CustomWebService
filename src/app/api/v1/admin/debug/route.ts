    ...
    try {
      ...
      return jsonResponse({
        ...
        data: {
          ...
          // **generate·regenerate·suggest 4종 = 6곳**을 403으로 막는다
          // — **신규 사용자가 제품을 아예 못 쓴다.**
          // (`deploy`는 2026-08-01 외부 배포 스택 제거로 사라졌고, **게시(publish)에는
          //  애초에 이 게이트가 없다.** 이 라우트도 대상이 아니다 — 관리자 키로만 보호된다)
          //
          // ⚠️ 이 숫자는 한때 **7곳**이라고 적혀 있었다. 근거로 인용한 `grep -rl ... src/app/api`가
          // **이 주석이 있는 파일 자신을 셌기 때문**이다(2026-08-08 정정).
          //
          // ⚠️⚠️ 그 정정문이 처방한 대체 명령도 **똑같이 자기 자신을 셌다**(같은 날 감사가 발견).
          // 주석이 그 문자열을 포함하니 당연한 일인데, 정정하면서 그걸 다시 놓쳤다.
          // 그 다음 처방은 import 까지 세어 12를 냈다 — **같은 주석에서 세 번 틀렸다.**
          //
          // 재확인 명령(2026-08-08 실행해 6을 확인한 형태 그대로다):
          //     grep -rn 'await assertEmailVerified(' src/app/api | grep -v admin/debug
          // 두 조각이 **둘 다 필요하다** — `await …(` 가 없으면 import 가 섞이고(12),
          // `grep -v` 가 없으면 이 주석 자신이 섞인다(7). `-l`(파일 목록)은 쓰지 않는다.
