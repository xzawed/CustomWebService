...
| 준비물 | 설명 |
|--------|------|
| **Google 계정** | Gmail 주소가 있으면 됩니다 (Google OAuth, Gemini API에 사용) |
...
6. **"Success"** 메시지가 나오면 성공입니다
   - 15개의 무료 API 정보와 7개의 기능 설정값이 입력되었습니다
...
> 그곳에서 로그인 처리를 완료한 뒤 대시보드 등으로 이동시킵니다.

---

## Step 6. Gemini API 키 발급 (약 3분)

> **Gemini란?** Google이 만든 AI 서비스입니다.
> 우리 서비스에서 "AI로 웹서비스 코드를 자동 생성"하는 기능에 사용됩니다.

### 6-1. Google AI Studio에서 API 키 발급받기

1. 새 브라우저 탭에서 **https://aistudio.google.com** 에 접속합니다
2. Google 계정으로 로그인합니다
3. 이용약관 동의 화면이 나오면 동의합니다
4. 왼쪽 메뉴 또는 상단에서 **"Get API key"** 를 클릭합니다
5. **"Create API key"** 버튼을 클릭합니다
6. 프로젝트 선택 팝업이 나오면:
   - 아무 프로젝트나 선택하거나, **"Create API key in new project"** 를 선택합니다
7. API 키가 생성됩니다 (예: `AIzaSyA-xxxxxxxxxxxxxxxxxxxx`)
8. **"Copy"** 버튼을 눌러 키를 복사합니다 → 메모장에 붙여넣기

### 6-2. .env.local 파일에 키 추가하기

1. `.env.local` 파일을 텍스트 편집기로 엽니다
2. `GEMINI_API_KEY=` 줄을 찾아서 `=` 뒤에 복사한 키를 붙여넣습니다:
   ```
   GEMINI_API_KEY=AIzaSyA-xxxxxxxxxxxxxxxxxxxx
   ```
3. 파일을 저장합니다 (Ctrl+S)
...
2. 아래 명령어에서 `YOUR_KEY` 부분을 방금 복사한 키로 교체한 뒤 실행합니다:
   ...
   ```
3. `"text": "Hello! ..."` 와 같은 응답이 돌아오면 정상 작동하는 것입니다
...
| 2 | **API 카탈로그** | 주소창에 `http://localhost:3000/catalog` 입력 → Enter → API 카드들이 목록으로 보이면 정상 (약 15개) | ☐ |
...
## Step 8. Vercel 배포 (약 10분)

> **Vercel이란?** 우리가 만든 서비스를 인터넷에 공개하여 누구나 접속할 수 있게 해주는 호스팅 서비스입니다.
> 무료로 사용 가능하며, GitHub와 연동하여 코드가 바뀌면 자동으로 다시 배포해줍니다.

### 8-1. Vercel 가입 및 프로젝트 연결하기

1. 새 브라우저 탭에서 **https://vercel.com** 에 접속합니다
2. **"Sign Up"** 또는 **"Start Deploying"** 버튼을 클릭합니다
3. **"Continue with GitHub"** 를 선택하여 GitHub 계정으로 가입/로그인합니다
4. 가입이 완료되면 대시보드가 나옵니다
5. **"Add New..."** 버튼을 클릭 → **"Project"** 를 선택합니다
6. **"Import Git Repository"** 섹션에서 GitHub 저장소 목록이 나옵니다
7. **"CustomWebService"** 저장소를 찾아 **"Import"** 버튼을 클릭합니다
   - 저장소가 보이지 않으면 **"Adjust GitHub App Permissions"** 를 클릭하여 저장소 접근 권한을 추가합니다

### 8-2. 환경변수 설정하기

> **환경변수란?** 앞서 `.env.local` 에 저장한 키들을 Vercel 서버에도 알려주는 것입니다.

1. **"Configure Project"** 화면에서 아래로 스크롤합니다
2. **"Environment Variables"** 섹션을 찾습니다
3. 아래 5개의 환경변수를 하나씩 추가합니다:

   **추가 방법**: 왼쪽 입력란에 이름(NAME), 오른쪽 입력란에 값(VALUE)을 입력 → **"Add"** 클릭
   ...
   | `GEMINI_API_KEY` | Step 6에서 복사한 Gemini API 키 |
   | `NEXT_PUBLIC_APP_URL` | 일단 `https://custom-web-service.vercel.app` 입력 (배포 후 실제 URL로 수정) |

4. 모든 환경변수를 추가한 뒤 **"Deploy"** 버튼을 클릭합니다
5. 빌드가 시작됩니다. 약 2분 정도 기다립니다
6. **"Congratulations!"** 화면이 나오면 배포 성공입니다!
7. 화면에 표시된 **배포 URL**을 복사합니다 (예: `https://custom-web-service-abc123.vercel.app`)
8. 이 URL을 브라우저에 입력하면 서비스가 인터넷에서 동작하는 것을 확인할 수 있습니다
...
1. Vercel 대시보드에서 방금 만든 프로젝트를 클릭합니다
2. 상단 탭에서 **"Settings"** 를 클릭합니다
3. 왼쪽 메뉴에서 **"Environment Variables"** 를 클릭합니다
4. `NEXT_PUBLIC_APP_URL` 항목을 찾아 오른쪽 **"..."** → **"Edit"** 클릭
5. 값을 실제 배포 URL로 변경합니다 (예: `https://custom-web-service-abc123.vercel.app`)
6. **"Save"** 클릭

**B. Supabase URL 설정 업데이트:**

1. **Supabase 대시보드** → **"Authentication"** → **"URL Configuration"**
2. **Site URL**을 Vercel 배포 URL로 변경합니다:
   ```
   https://custom-web-service-abc123.vercel.app
   ```
3. **Redirect URLs** 에 아래 주소를 추가합니다 (**"Add URL"** 클릭):
   ```
   https://custom-web-service-abc123.vercel.app/callback
   ```
...
3. **Homepage URL**을 Vercel 배포 URL로 변경합니다:
   ```
   https://custom-web-service-abc123.vercel.app
   ```
...
2. 상단 탭에서 **"Deployments"** 를 클릭합니다
3. 가장 최근 배포 항목 오른쪽의 **"..."** 메뉴 → **"Redeploy"** 를 클릭합니다
4. **"Redeploy"** 버튼을 클릭하여 확인합니다
5. 약 2분 후 재배포가 완료됩니다
...
| 작업 | 설명 | 진행 방법 |
|------|------|----------|
| **Vercel 토큰 설정** | 사용자가 만든 서비스를 자동으로 인터넷에 배포하는 기능을 활성화합니다 | Vercel 대시보드 → Settings → Tokens → 토큰 생성 → `.env.local`과 Vercel 환경변수에 `VERCEL_TOKEN`, `VERCEL_ORG_ID` 추가 |
...
| 작업 | 설명 | 진행 방법 |
|------|------|----------|
| **커스텀 도메인** | `custom-web-service.vercel.app` 대신 `myservice.com` 같은 주소를 사용합니다 | 도메인 구매(가비아, Cloudflare 등) → Vercel → Settings → Domains → 도메인 연결 → DNS 설정 |
...
| Step 6 | AI(Gemini) API 키 발급 | 약 3분 |
...
- Step 6에서 발급한 **Gemini API 키**가 `.env.local`에 정확히 입력되었는지 확인하세요
- Gemini API 키가 유효한지 Step 6-3의 테스트 방법으로 확인해보세요

### Q. Vercel 배포 후 로그인이 안 돼요
- Step 8-3의 **URL 업데이트**를 모두 완료했는지 확인하세요
- 특히 Supabase의 **Site URL**과 **Redirect URLs**가 Vercel 배포 URL로 변경되었는지 확인하세요
