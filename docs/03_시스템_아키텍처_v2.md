...
│ CatalogRepo  │  │ ├──────────────┤ │  │ ├──────────────┤ │
│ CodeRepo     │  │ │GeminiProvider│ │  │ │VercelDeploy  │ │
│ OrgRepo      │  │ │OpenAiProvider│ │  │ │NetlifyDeploy │ │
│              │  │ │OllamaProvider│ │  │ │GHPagesDeploy │ │
│  ↓           │  │ └──────────────┘ │  │ └──────────────┘ │
│ Supabase     │  │                  │  │                  │
...
export interface TemplateMetadata {
  ...
  description: string;
  category: string;
  compatibleApiCategories: string[];
  thumbnail?: string;
}
...
export interface ICodeTemplate {
  ...
  canHandle(apis: ApiCatalogItem[], context: string): boolean;
  generate(apis: ApiCatalogItem[], context: string): TemplateResult;
  getPromptEnhancement(): string;  // AI 프롬프트에 추가할 힌트
}
```
...
[Repository Layer] 각 Repository가 Supabase 클라이언트로 DB 접근
    │
    ▼
[Provider Layer] AiProviderFactory → GeminiProvider → Gemini API 호출
```
...
| 새 배포 플랫폼 추가 (Railway) | Provider만 | 새 클래스 1개 + Factory 등록 |
