...
### 현재 구현 상태
1. **xAI Grok API** ✅ (기본 Provider)
   - 장점: 높은 코드 생성 품질, OpenAI 호환 API
   - 구현: `GrokProvider.ts` (OpenAI SDK 사용)
   - 모델: `grok-3-mini`

### 확장 가능 Provider (미구현, Factory 구조 준비됨)
2. **OpenAI API (GPT)** - `AiProviderFactory`에 등록 가능
3. **Ollama (로컬 LLM)** - `AiProviderFactory`에 등록 가능

### Provider 선택 전략 (AiProviderFactory)
```
사용자 요청 → AiProviderFactory.getBestAvailable()
                 ├── Grok 사용 가능 → GrokProvider 사용
                 └── 사용 불가 → 다음 우선순위 Provider 시도
```

### 구현 파일
- `src/providers/ai/IAiProvider.ts` - Provider 인터페이스
- `src/providers/ai/GrokProvider.ts` - Grok 구현
- `src/providers/ai/AiProviderFactory.ts` - Provider 팩토리
