...
| C4 | **쪼개서 볼 것**: (a) Sentry SaaS 도입 → ❌ **제외(2026-08-01)** (유료·Slack-only 결정 유지) (b) 미사용 `@sentry/nextjs` 의존성 + config 3파일 **삭제 정리** → 무료 cleanup, **액션 가능** | S | (b)만 착수 가능 |
...
| 순 | 항목 | 왜 이 순서인가 |
|---|---|---|
| 1 | **C4(b)** — 미사용 `@sentry/nextjs`(`^10.68.0`) + config 3파일 + `instrumentation.ts` 참조 정리 | 유일하게 **혼자 끝낼 수 있는** 코드 작업. 번들·감사 표면만 차지한다. Sentry SaaS 도입은 ❌ 제외 |
| 2 | **B1 · B3(a) · B5** — `AUTH_URL` 등록 · 무료 키 발급 · Unsplash 심사 | 전부 **오너 계정 조치**. 코드가 대신 못 한다. 비용 아님 (~~B6~~은 해결됨) |
| 3 | C3 · C5 · C7 · C8 · C9 | 소형 위생 작업. 급하지 않다 |
