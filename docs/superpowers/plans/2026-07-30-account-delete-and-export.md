...
사용자 결정: **`platform_events` 참조 끊기 + payload 익명화**, **게시 사이트 자동 연쇄 삭제**.
재가입은 하드 삭제로 이메일이 해제되어 자동 허용된다.

착수 중 **선행 결함**을 하나 더 발견해 함께 고쳤다 — `getAuthUser`가 DB를 조회하지 않아
삭제된 계정의 JWT가 유령 세션으로 살아 있었다. 구현·근거는
[ADR](../../decisions/2026-07-30-account-delete-and-export.md) 참조.
