...
## ⚠️ 추가 — 과거 반대 결정 확인 필요 (2026-07-30 보강)

세션 메모리에 **2026-04-27자 "SLACK_WEBHOOK_URL / errorRateMonitor 사용 안 함" 결정**이 남아 있다.
이 계획은 그 결정과 반대 방향이다.

당시 근거는 기록돼 있지 않고 ADR도 없다. 그 사이 서비스가 공개 셀프서비스 가입 + 실사용자
운영 단계로 바뀌었으므로 판단 근거가 달라졌을 가능성이 높다. 그래도 **모르고 진행하면 이미 내린
결정을 조용히 뒤집는 셈**이므로, 착수 시 사용자에게 먼저 확인한다:

- 2026-04-27 결정을 뒤집는 것이 맞는가
- 뒤집는다면 근거를 ADR로 남길 것

상세: [#220 코멘트](https://github.com/xzawed/CustomWebService/issues/220#issuecomment-5120693118)
