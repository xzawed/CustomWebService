...
> **언제 읽나**: `/data/app.db` 를 복구하거나 백업(`scheduleBackups`)·WAL 체크포인트·`-wal`/`-shm` 처리를 손댈 때. **복구 성공 판정은 크기가 아니라 행 수다**
...
## ⛔ 시작 전에 반드시 읽을 것 — WAL 모드 메커니즘 불변조건

파일 크기 스냅샷(“main 4096B · WAL에 실데이터”)은 **특정 시점 관측**이지 영원한 사실이 아니다.
불변조건은 메커니즘이다:

1. WAL 모드에서는 **커밋된 데이터가 체크포인트 전까지 `-wal`에만 존재할 수 있다.**
2. 따라서 **살아 있는 main을 두고 `-wal`/`-shm`만 지우면 안 되고**, main만 교체하고 옛 WAL을 남기면
   **조용히 잘못된 상태로 재생**된다 (`integrity_check`는 `ok`를 반환한다).
3. `.backup()` 산출물은 **자기완결**이다 — 복구 = 그 파일 설치 + WAL/SHM 잔존 제거.
4. **파일 크기는 건강 지표가 아니다.** 깨끗한 종료 없이 오래 뜨면 WAL이 autocheckpoint 임계
   (기본 1000페이지 ≈ 3.9MB)까지 자랄 수 있고, 복구 직후엔 반대로 main이 크고 WAL이 비어 있다.

이 때문에 두 가지 실수가 각각 치명적이다:

| 실수 | 결과 |
|---|---|
| **살아 있는 main을 두고 WAL만 지운다** | 체크포인트 전 커밋이 있으면 **데이터 소실** |
| **백업본으로 app.db만 덮어쓰고 옛 WAL을 남긴다** | **복구가 조용히 무효화된다.** 남아 있던 WAL이 복구본 위로 재생되어 손상 상태로 되돌아가고, `PRAGMA integrity_check`는 여전히 `ok`라고 답한다 (실측 확인) |

**따라서 app.db 교체와 WAL/SHM 제거는 반드시 한 세트로 수행한다.**
**성공 판정은 행 수 대조**이며, `integrity_check`나 파일 크기가 아니다.

> empty-main/fat-WAL(2026-07-30), fat-main/empty-WAL(2026-07-31 리허설 후) 같은 수치는
> 아래 [리허설 기록](#리허설-기록)에 **시점 관측**으로만 남긴다.
...
| 오해 | 사실 |
|---|---|
| "WAL은 캐시니까 지워도 된다" | 체크포인트 전 커밋은 `-wal`에만 있을 수 있다. **살아 있는 main을 두고 WAL만 지우면 데이터가 날아갈 수 있다.** main 교체와 WAL/SHM 제거는 한 세트 |
| "지금 main이 크니 WAL은 무시해도 된다" | 다음 체크포인트 전엔 다시 WAL에만 쌓인다. **크기는 건강 지표가 아니다** |
...
- **볼륨 자체 손실은 이 런북의 on-volume 덤프만으로는 복구할 수 없다.** `/data/backups`가 `/data`와 같은 볼륨이라 함께 사라진다.
  계층 전체는 [operations.md §3.4 DR 계층](operations.md):
  1. on-volume 덤프(이 런북) — 논리 손상·잘못된 마이그레이션·실수 삭제. **자동·운영 중**, 리허설 검증됨
  2. `GET /api/v1/admin/backup/latest` — **유일한 무료 오프-볼륨 경로**. 사람이 **미리** 로컬에 받아 둔 경우에만 볼륨 손실을 막는다(자동화·강제 스케줄 없음)
  3. Railway 볼륨 백업·관리형 스토리지·Litestream→S3 — ❌ **제외(2026-08-01)**. 비용이 드는 신규 완화는 구현 대상이 아니다. 오너 액션 대기가 아니라 **하지 않기로 한 결정**. `SQLITE_OFFSITE_BACKUP_URL` 시임은 코드에 있으나(기본 no-op) **수신기를 제공·권장하지 않는다**
- **볼륨이 사라지고 오프라인 사본이 없으면 복구 절차는 없다** — 빈 bootstrap만 남는다. 그 잔여 위험을 수용한다.
...
| 2026-07-30 | 프로덕션 (읽기 전용 조사) | `/data` 파일 구성·백업 7개 존재·백업본 무결성 `ok`·**그 시점** app.db 단독 테이블 0개 확인 |
| **2026-07-31** | **프로덕션 (쓰기 리허설)** | **성공.** 위 "복구 절차" 1)~5)를 그대로 실행. 총 5분, 롤백된 데이터 0건. 아래 실측 참조 |

### 시점 관측 — main/WAL 크기 (**불변조건이 아님**)

아래는 특정 시각의 `ls` 결과다. 메커니즘 설명에 쓰지 말고, “그때 이렇게 보였다”로만 읽는다.

| 시점 | `app.db` | `app.db-wal` | 해석 |
|---|---|---|---|
| 2026-07-30 프로덕션 조사 | **4096B** · 테이블 0 | **~1.3–1.4MB** | 장기 실행·체크포인트 미도달 → 실데이터가 WAL에 누적된 **그 시점** 상태 |
| 2026-07-31 리허설 직후 | **417792B** | **0B** | `.backup()` 복구 후 자기완결 main + 빈 WAL. 리허설 전 스냅샷은 `/data/incident-20260731-101945/` |
| 2026-08-02 프로덕션 점검 | **417792B** | **0B** | 리허설 이후 형태 유지. 실데이터가 main에 있는 **그 시점** 상태 |
...
- `app.db-wal`의 mtime이 **대상 백업 시각보다 앞서면** 그 백업 이후 쓰기가 0건인 **신호**다
  (WAL 모드에서 커밋은 보통 `-wal`로 가므로 mtime이 마지막 쓰기에 가깝다).
- **교차 확인이 필수다.** 복구 직후 0바이트 WAL이면 mtime이 뒤처지거나 의미가 약해질 수 있다.
  **백업·main mtime 또는 행 수 대조**로 함께 본다. WAL mtime 단독으로 단정하지 말 것.
- 리허설 당시 실측(2026-07-31, 복구 전): WAL mtime `07-30 14:31` < 백업 `07-30 15:44` →
  **롤백 데이터 0건**, 사전 재배포 생략. 행 수 교차 확인(12테이블 전부 일치).
...
| **데이터 롤백 범위** | 복구 시점 = 사용할 백업의 시각. 그 이후 쓰기는 되돌아간다. WAL mtime + 백업/main mtime 또는 행 수로 폭을 미리 잴 것 |
