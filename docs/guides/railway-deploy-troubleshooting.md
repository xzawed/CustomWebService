# Railway 배포 실패 진단 — 조용한 미배포·미검증 4종 (2026-08-05~07 실측)
...
## 공통 위험 — 넷 다 "장애"로 보이지 않는다

네 실패 모두 **서비스는 멀쩡히 떠 있고 health도 200**이다.
결과는 조용히 *"고쳤는데 안 바뀐다"* 상태다. 배포 목록을 직접 보지 않으면 알 수 없고,
그래서 진짜 장애보다 발견이 늦다. **③④는 배포 목록으로도 안 보인다** — 런 목록을 봐야 한다. 특히 ④는 배포가 SUCCESS 인데도 그 커밋이 CI 를 통과한 적이 없을 수 있다.

---

## 판별표 — `FAILED`를 오해하지 말 것

**병합했으면 배포 status가 `SUCCESS`인지 반드시 확인한다.** 조용한 미배포·미검증이 4종 있고, 넷 다
서비스는 멀쩡히 떠 있고 health도 200이라 **배포 목록(③④는 런 목록)을 보지 않으면 모른다.**
...
| 신규 커밋 · 배포 `SUCCESS` 인데 **그 SHA의 성공 런이 0건** | 연속 머지로 CI가 **취소**됐다. 배포는 됐지만 **검증되지 않았다** → 아래 ④ (확인 명령 포함) |
...
> 2026-08-07에 `ci.yml`에 `workflow_dispatch`를 추가해 이 경로가 생겼다. 그 전에는 **수동 실행이
> 불가능**해(트리거가 `push`·`pull_request`뿐) 커밋을 하나 더 올리는 것이 유일한 방법이었다.
>
> ⚠️ **디스패치 대상 ref의 `ci.yml`에 트리거가 있어야 한다.** GitHub은 워크플로 파일을 *그 ref에서*
> 읽으므로, 트리거 추가 이전에 분기한 브랜치는 여전히 422로 거부된다(2026-08-07 실측).
>
> ⚠️ **디스패치는 같은 ref의 진행 중 런을 취소시킨다**(아래 ④). 검증하려다 검증을 지우지 말 것.

---

## ④ 연속 머지가 앞 커밋의 CI를 취소시킨다 (2026-08-07 실측)

`concurrency`는 `group: ${{ github.workflow }}-${{ github.ref }}`다. main에 **연달아 머지**하면
뒤 커밋의 푸시가 앞 커밋의 런을 취소한다. PR 브랜치에서는 옳은 동작이지만 —
**main은 커밋 하나하나가 배포 대상**이고, 취소된 커밋도 Railway는 배포한다.

즉 *"CI 통과했다"가 커밋 단위로는 거짓*이 될 수 있다. 실측:

| main 커밋 | 성공 런 | |
|---|---|---|
| `183bca6` | **0건** | 취소 ×2 (뒤 푸시 + 내가 돌린 디스패치) |
| `38f531b` | 1건 | ✅ |
| `9199cec` | **0건** | 취소 |
| `8dbbc3c` | 1건 | ✅ |
| `6f2c7c1` | **0건** | ③ 웹훅 장애 |

**6개 중 성공 런이 있는 것은 2개뿐이었다.**

**대응(2026-08-07)**: `cancel-in-progress: ${{ github.ref != 'refs/heads/main' }}` —
PR은 그대로 취소하고 **main만 취소하지 않는다.** 대가는 CI 분(minutes)이고, 검증 공백보다 싸다.

**임의의 커밋이 CI를 통과했는지 확인하는 명령**:

```bash
SHA=$(git rev-parse origin/main)
gh run list --branch main --limit 25 --json headSha,conclusion \
  --jq "[.[]|select(.headSha==\"$SHA\" and .conclusion==\"success\")]|length"
# 0 이면 그 커밋은 CI 통과한 적이 없다 — 배포 status 가 SUCCESS 여도 그렇다
```
