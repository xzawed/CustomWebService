...
// 로직:
// 1. 인증 확인
// 2. 프로젝트 소유권 확인 + 기존 코드 조회
// 3. 재생성 횟수 제한 확인 (maxRegenerationsPerProject)
// 4. 일일 생성 횟수 확인 (재생성도 일일 한도에 포함)
// 5. buildRegenerationPrompt(이전코드, feedback)로 프롬프트 구성
// 6. AI 코드 생성 → 파싱 → 검증
// 7. 새 버전으로 generated_codes 저장 (metadata.userFeedback 포함)
// 8. projects 상태 'generated'로 업데이트
// 9. CODE_GENERATED 이벤트 발행
```
