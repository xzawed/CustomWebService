...
- `test:e2e`는 **env 없이 그냥 돈다.** `playwright.config.ts`가 `AUTH_SECRET`·`AUTH_URL`·
  `SQLITE_PATH`(`.tmp/e2e-app.db`)·`DB_PROVIDER`를 자체 폴백으로 공급하고, `webServer`가
  시드 후 서버를 띄운다. 필요한 것은 **빌드 산출물(`pnpm build`)과 Playwright 브라우저**뿐이다.
  실측(2026-08-07): `.env.local`을 **치우고** 돌려 **43/43 통과**.
  이 줄은 원래 "실 백엔드 env가 있어야 돈다"였다. **폴백이 들어온 2026-08-01(`8399a00c`)에
  거짓이 됐고, 8/06 문서 감사(#279)가 "파생 불가 사실"이라며 그대로 보존했다** — 감사가 검사한
  것은 *`package.json`에서 파생되는가*였지 *지금도 참인가*가 아니었다. **문서 감사는 파생성만
  보지 말고 명령을 한 번 돌려볼 것.** 6일이 걸린 게 아니라, 돌려봤으면 6초였다
