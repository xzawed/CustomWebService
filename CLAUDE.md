...
- **플랫폼 키 = Railway sealed 변수**: 키 의존 API의 env 키(`API_KEY_*`)는 Railway에서 **sealed**이라 `railway run`/CLI에 주입되지 않는다(배포 런타임 전용). 따라서 `pnpm keys:verify`를 로컬 `railway run`으로 돌리면 전부 가짜 MISSING — **봉인 키 유효성은 배포 컨텍스트 안에서만 검증 가능**. 이를 위해 관리자 진단 엔드포인트 **`GET /api/v1/admin/keys-verify`**(ADMIN_API_KEY 보호, 로직 `src/lib/catalog/keyCheck.ts`)를 배포 환경에서 호출한다. 또한 카탈로그 `auth_config.env_var` 이름이 실제 Railway 변수명과 일치해야 함(불일치 시 키가 있어도 401). 예: **카카오 검색이 존재하지 않는 `API_KEY_KAKAO`를 참조**했던 버그 → `API_KEY_F1EC6F97`로 정정(2026-06-21)
